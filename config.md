...
The settings dialog groups common review choices first and keeps additional controls in Advanced. Anki stores these values in `config.json`.
...
- Pack status and Download/Update, Pause/Resume, Cancel, Verify, and Remove controls are always visible in the main settings view.
...
- `audio_backend`: playback mode. The default is `local_audio_then_tts`, which uses Custom audio first, then High Quality Downloaded Pack audio, followed by Standard text-to-speech when needed. High Quality Downloaded Pack is the user-facing label for all high-quality neural audio, whether bundled or downloaded.
...
- `show_save_button`: show Save pronunciation in both the details popup and quick pronunciation card.
