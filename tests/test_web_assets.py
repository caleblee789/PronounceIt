...
sandbox.window.PronounceIt.show({
  ...
  audioSource: "azure",
  audioSourceLabel: "High Quality Downloaded Pack",
  audioAvailable: true,
  request,
  rect: request.rect,
});
...
if (!badge || badge.textContent !== "High Quality Downloaded Pack") {
  throw new Error("High Quality Downloaded Pack was not shown");
}
sandbox.window.PronounceIt.spoken({
  ok: true,
  audioSource: "live",
  audioSourceLabel: "Standard text-to-speech",
  term: "clozapine",
});
if (badge.textContent !== "Standard text-to-speech" || !badge.className.includes("live")) {
...
if (body.querySelector(".pronounceit-source").textContent !== "Standard text-to-speech") {
